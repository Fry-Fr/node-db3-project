...
async function findById(scheme_id) { // EXERCISE B
  ...
  */
    ...
    const objList = await db('schemes')
      .leftJoin('steps', 'schemes.scheme_id', 'steps.scheme_id')
      ...
      return {
        "scheme_id": newObj.scheme_id,
        "scheme_name": newObj.scheme_name,
        "steps": objList[0].step_id ? objList.map(obj => {
            return {
              "step_id": obj.step_id,
...
async function addStep(scheme_id, step) { // EXERCISE E
  ...
  */
  ...
  return db('steps').orderBy('steps.step_number', 'asc').where({ "scheme_id": scheme_id });
}
